...
Log in to [OpenHands Cloud](https://app.all-hands.dev) under your team org and send the following prompt in a new conversation. Replace `YOUR_ORG` with your GitHub organization name:
...
Filter: glob(repository.full_name, 'YOUR_ORG/*') && (label.name == 'qa-this' || (!label && !pull_request.draft && pull_request.author_association != 'FIRST_TIME_CONTRIBUTOR' && pull_request.author_association != 'FIRST_TIMER' && pull_request.author_association != 'NONE'))
...
For a more detailed automation setup with progress comments and session links, see the [Automated Code Review automation guide](/openhands/usage/use-cases/code-review#automate-this) — the same pattern applies to QA.
