...
pull_request.ready_for_review, and pull_request.labeled (with label "qa-this")
for repos matching glob(repository.full_name, 'YOUR_ORG/*').
Exclude draft PRs (!pull_request.draft) from the opened/ready_for_review triggers.
