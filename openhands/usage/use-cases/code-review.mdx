...
|-------|-------------|----------|---------|
| `review-agent-mode` | Review backend: `openhands` for the standard SDK agent or `acp` for an ACP-compatible agent server | No | `openhands` |
| `llm-model` | LLM model to use | Yes | - |
| `acp-command` | Command used to start the ACP server. Required when `review-agent-mode` is `acp`. Examples: `npx -y @zed-industries/codex-acp@0.12.0`, `codex-acp`, `claude-agent-acp`, `npx -y @agentclientprotocol/claude-agent-acp` | Yes for ACP mode | `''` |
...
| `llm-api-key` | LLM API key. Required when `review-agent-mode` is `openhands`; ignored in ACP mode. | Yes for OpenHands mode | - |
...
`review-agent-mode: acp`. In this mode, OpenHands still loads the review skills
