...
[OpenHands Automations](/openhands/usage/automations/overview) is an event-triggered automation system that can replace per-repo GitHub Actions workflows. You define the trigger once and it covers all repositories matching your filter — no per-repo workflow files needed.
...
The `$AUTOMATION_EVENT_PAYLOAD` variable contains the full GitHub webhook event as JSON. Both variables are provided automatically — no additional configuration is needed.
</Note>
