...
- You are using OpenHands Cloud (SaaS mode)

To access it:

1. Navigate to the Integrations page in OpenHands Cloud
...
The webhook management table displays GitLab groups and projects not under any group that are accessible to OpenHands. Projects that belong to a group are not shown individually, as a group-level webhook can capture events from all projects within that group. For each resource, you can see:
