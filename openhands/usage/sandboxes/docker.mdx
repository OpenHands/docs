...
<Note>
  <code>OH_WEB_URL</code> does **not** control these sandbox URLs. It only sets
  the agent-server's <code>root_path</code> and adds the origin to the sandbox's
  CORS allow-list. Setting it will not change the host/port the browser uses to
  reach a sandbox.
</Note>
