...
To resume a paused conversation, use the sandbox resume endpoint. This restarts the sandbox environment for a conversation that's in a paused state.
...
<Tabs>
  ...
  <Tab title="Python (with requests)">
    ...
    api_key = "YOUR_API_KEY"
    sandbox_id = "sandbox-abc123"  # Get this from conversation details
    url = f"https://app.all-hands.dev/api/v1/sandboxes/{sandbox_id}/resume"
    ...
    print(f"Sandbox resumed: {result}")
    ```
  </Tab>
  <Tab title="TypeScript/JavaScript (with fetch)">
    ```typescript
    const apiKey = "YOUR_API_KEY";
    const sandboxId = "sandbox-abc123";  // Get this from conversation details
    const url = `https://app.all-hands.dev/api/v1/sandboxes/${sandboxId}/resume`;
    ...
    async function resumeSandbox() {
      try {
        ...
        const result = await response.json();
        console.log("Sandbox resumed:", result);
...
<Note>
  You can get the `sandbox_id` from the conversation details using the 
  `GET /api/v1/app-conversations/{id}` endpoint. The sandbox must be in a 
  paused state to resume it.
</Note>
