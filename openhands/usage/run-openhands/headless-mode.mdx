...
```bash
# Start with a specific task in headless mode
openhands --headless --task "Fix the bug in main.py"

# Run with experimental UI and a file in headless mode
openhands --headless --exp --file src/app.py --task "Add error handling to this file"

# Run in headless mode with auto-approval
openhands --headless --always-approve --task "Run tests and fix any failures"

# Exit without confirmation prompts
openhands --headless --exit-without-confirmation --task "Generate documentation for the project"
```
