...
If OpenHands is running on a Mac or Linux machine at home, you can access the web interface from your phone, tablet, or any other device on the same network.

### Using Tailscale (Recommended for Personal Use)

[Tailscale](https://tailscale.com/) creates a private network between your devices — no port forwarding, no exposure to the public internet.

1. Install Tailscale on both your computer (where OpenHands runs) and your phone/tablet.
2. Sign in with the same account on both devices.
3. Find your computer's Tailscale IP (shown in the Tailscale app, typically `100.x.y.z`).
4. Open `http://<tailscale-ip>:3000/` in your phone's browser (use the port OpenHands is running on).

<Tip>
If you previously accessed OpenHands from the same browser on a different address, clear the site's localStorage first — the stored backend configuration may point at `127.0.0.1`, which is localhost *on your phone*, not your computer.
</Tip>

### Using ngrok (For Remote Access)

If you need access from outside your local network, [ngrok](https://ngrok.com/) creates a secure tunnel with a public URL. You can add Google OAuth or other authentication to prevent unauthorized access:
...
<Warning>
When exposing OpenHands over the internet, always use authentication. ngrok supports OAuth, IP allowlists, and other access controls. Without authentication, anyone with the URL can use your OpenHands instance and the LLM API keys configured in it.
</Warning>
