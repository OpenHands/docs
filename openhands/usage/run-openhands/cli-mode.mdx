...
<Tabs>
  <Tab title="Using uv (recommended)">
    ...
    **Upgrade OpenHands:**
    ...
    <Accordion title="Create shell aliases for convenience">

    If you want shorter commands, add the following to your shell configuration file (`.bashrc`, `.zshrc`, etc.):

    ```bash
    alias oh="openhands"
    ```

    After adding this line, reload your shell configuration with `source ~/.bashrc` or `source ~/.zshrc` (depending on your shell).
