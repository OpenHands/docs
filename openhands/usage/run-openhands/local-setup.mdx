...
#### Option 3: Using Docker Without Docker Socket (Process Runtime)

If you prefer not to mount the Docker socket for security reasons, you can use the process runtime which runs
the agent directly inside the OpenHands container without creating additional Docker containers.
...
<Accordion title="Docker Command Without Socket (Click to expand)">
