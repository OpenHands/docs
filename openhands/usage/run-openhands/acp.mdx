...
#### MCPs Not Being Recognized

If you're using OpenHands in Zed with the ACP plugin and one or more of your Model Context Protocol (MCP) servers are not being recognized:
...
3. Review the logs for any errors or warnings related to your MCP configuration
4. Restart a new conversation to reload the MCP connections

This debug log will help you identify configuration issues, connection problems, or errors in your MCP server setup.

<Note>
If the debug logs show persistent issues, check that your MCP servers are properly configured in your Zed settings and that the server commands are executable from your system PATH.
</Note>
