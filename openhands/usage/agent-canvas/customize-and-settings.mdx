...
`Customize` opens on `MCP Servers`.
...
The same page shows the installed Agent Canvas version and a **Check for updates** button. When an update is available, Agent Canvas also shows a version tile in the main sidebar. Select either entry point to open the update dialog, which includes release notes and installation commands for npm and Docker.
