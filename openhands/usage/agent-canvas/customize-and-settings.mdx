...
On local backends, the `LLM` page also includes an `Available Profiles` area for saved profiles, and a **Provider Connections** section for connecting a vendor once and reusing its key across models (see [Connect a Provider](/openhands/usage/agent-canvas/provider-connections), preview).
