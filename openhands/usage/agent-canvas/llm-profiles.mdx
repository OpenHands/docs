...
<Note>
To reuse a single API key across every model a vendor offers, connect the
provider once instead of pasting the key into each profile. See
[Model providers](/openhands/usage/agent-canvas/provider-connections)
...
Open `Settings > Model providers` to add a reusable LLM profile. Use the **Basic** tab for a provider and model available in the dropdowns. Use the **Advanced** tab when you need to enter a model name and base URL directly. Use the **All** tab to view and customize the full set of model configuration fields.
