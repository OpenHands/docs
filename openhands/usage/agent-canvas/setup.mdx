...
Agent Canvas is also available as an all-in-one Docker image that bundles the frontend, agent server, and automation backend.
...
The agent can access any project folder under the `/projects` mount. Settings, secrets, and conversation history persist in the `~/.openhands` mount.

An API key and secret key are auto-generated on first run and persisted across restarts. To provide your own:

```bash
docker run -it --rm \
  -p 8000:8000 \
  -e LOCAL_BACKEND_API_KEY=<your-key> \
  -v ~/.openhands:/home/openhands/.openhands \
  -v ~/projects:/projects \
  ghcr.io/openhands/agent-canvas:latest
```
