...
- **Reusable agent profiles**: choose OpenHands or an ACP agent profile when you start a conversation, then keep common agent setups available from `Settings > Agent`.
- **Plugin management**: browse, install, enable, disable, and attach plugins from the Agent Canvas UI when using a local backend.
...
| **Agent Profile** | A reusable agent configuration for new conversations. Profiles can run OpenHands or an ACP agent such as Claude Code, Codex, or Gemini CLI. |
...
| **Plugin** | A reusable package of agent components, such as skills, MCP servers, hooks, commands, or agent definitions. |
...
- [Agent Profiles](/openhands/usage/agent-canvas/agent-profiles) — Manage reusable agent configurations for new conversations.
- [Conversations](/openhands/usage/agent-canvas/conversations) — Branch from previous messages and continue from alternate paths.
- [Plugins in Agent Canvas](/openhands/usage/agent-canvas/plugins) — Manage and attach plugins from the UI.
...
Use `Command-K` on macOS or `Ctrl-K` on Windows and Linux to open the command palette. The palette provides quick access to navigation, settings, and common local actions without leaving the keyboard.
