...
<Note>
To reuse one API key across every model a vendor offers instead of pasting it
into each profile, connect the provider once from `Settings > Model providers`. See
