...
3. Enter the provider details:
   - **Display name** — optional name shown on the provider card.
   - **Base URL** — optional custom endpoint, such as an OpenAI-compatible
     gateway or enterprise proxy.
   - **Wire API** — choose `Auto`, `Chat Completions`, or `Responses`.
   - **API key** — the key issued by that provider or gateway.
...
the UI — the connection only reports whether a key is set. The provider record
stores non-secret endpoint settings, including base URL, wire API, and custom
headers. Rotating a key later updates the secret that every profile created from
