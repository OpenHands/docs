...
description: Store one API key and base URL as a named provider connection, then link multiple LLM profiles to it so they share the key.
---
...
LLM profile would otherwise carry inline: an **API key** and a **base URL**.
Instead of pasting the same key into every profile that uses a provider,
...
|---|---|---|
| Holds a key | Yes — one key and base URL shared by linked profiles | Yes — inline, unless linked to a connection |
...
- **Provider presets, wire-API selection, and custom headers** — the form is a
  name, a free-text provider, a key, and a base URL.
