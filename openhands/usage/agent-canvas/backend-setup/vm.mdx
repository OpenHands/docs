...
| **Backend only + ngrok** | `agent-canvas --backend-only --public` and `ngrok http 8000 --url https://your-domain.ngrok-free.app` | Use your ngrok domain as the backend URL. Do not add ngrok OAuth for this mode; rely on `LOCAL_BACKEND_API_KEY`. |
...
Every ngrok account—including the free plan—comes with a static domain that stays the same across restarts, so you can save it once in **Manage Backends** and keep reusing it. Claim or view your domain on the [**Domains**](https://dashboard.ngrok.com/domains) page of the ngrok dashboard.

On the free plan you get an ngrok-branded domain (for example `your-domain.ngrok-free.app`). On a paid plan you can bring your own custom domain instead, such as `canvas.example.com`.

On the VM, in a second terminal or tmux pane, start ngrok on your domain:
