...
description: Run Agent Canvas on a VM or dedicated machine and access it remotely.
---

This guide walks through deploying Agent Canvas on a virtual machine or dedicated host so you can reach it from anywhere.

<Warning>
  Agent Canvas drives an agent that can read and write the host filesystem, execute shell commands, and access the network. Treat the machine like any host that holds production credentials.
</Warning>
...
The `--public` flag means the API key is **not** baked into the frontend. Anyone who opens the UI must enter the key before they can interact with the agent.
...
## 5. (Optional) Connect from Your Local Agent Canvas

You don't have to use the remote UI directly. If you already run Agent Canvas locally, add the remote machine as a backend:
