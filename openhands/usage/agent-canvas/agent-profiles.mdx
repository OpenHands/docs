...
Before starting a conversation, open the `+` tools menu in the chat launcher and select `Switch agent profile`. Choose a profile to use it for the new conversation. In a blank conversation, choosing a profile starts a replacement conversation with that profile.

The model selector remains visible in the chat launcher. It selects the model for the current profile; it does not switch which agent runs the conversation. Existing conversations keep the agent profile they were launched with.
