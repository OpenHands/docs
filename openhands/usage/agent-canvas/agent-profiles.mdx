...
When the connected Agent Server advertises the `profile_secret_scope_v1` capability, the profile editor in `Settings > Agent` includes a **secret scope** picker. Use it to control which saved secrets an agent profile can access:
