...
- The identity (user) the automation runs as, shown in the configuration section on cloud backends
...
<Note>
  On cloud backends, only the automation's creator can re-enable a disabled automation. Other members with manage permissions can disable or delete an automation, but cannot turn it back on.
</Note>
