...
Creating an automation is governed by the same view permission as the list itself, so on cloud backends organization members who can view automations also see the add and import entry points. Editing or disabling an existing automation still requires manage permissions or being the automation's creator.
