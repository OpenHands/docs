...
2. **Configure settings** (first run only):
   ...
   **Option A: Use OpenHands Cloud** (recommended)
   ```bash
   openhands login
   ```
   This fetches your settings from OpenHands Cloud. First-time users get **$10 in free credits**.
   
   **Option B: Configure manually**
   The CLI will prompt you to enter your LLM provider and API key. You can also access settings anytime by pressing `Ctrl+P` and selecting **Settings**.
...
<CardGroup cols={2}>
  ...
  <Card title="Settings" icon="gear" href="/openhands/usage/cli/settings">
