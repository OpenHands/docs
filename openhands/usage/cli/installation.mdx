...
Before installing the OpenHands CLI, ensure your system meets the requirements:
...
<AccordionGroup>
  ...
  - [Docker Desktop](https://docs.docker.com/desktop/setup/install/linux/) installed and running

  </Accordion>
  <Accordion title="Windows">

  - Windows 10/11 with [WSL](https://learn.microsoft.com/en-us/windows/wsl/install) installed
  - Ubuntu (or another Linux distribution) installed via WSL
  - [Docker Desktop](https://docs.docker.com/desktop/setup/install/windows-install/) with WSL integration enabled
...
<Note>
**Windows users:** All commands below should be run inside the WSL terminal (Ubuntu). Use `wsl -d Ubuntu` in PowerShell or search "Ubuntu" in the Start menu to access the Ubuntu terminal.
</Note>
