...
When **Iterative Refinement** mode is enabled, the CLI automatically prompts the agent to review and improve its work if the critic predicts a low probability of task success. This creates a bounded retry mechanism where the agent reviews and improves its work when the critic predicts low success probability.
...
**Example with refinement threshold set to 80%** — the agent must achieve a higher score to pass, triggering more refinement cycles:
