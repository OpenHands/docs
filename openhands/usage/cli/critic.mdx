...
Refinement can also be triggered when the critic detects **specific issues** (e.g., insufficient testing, missing error handling) above the issue threshold (default: 75%), even if the overall score is acceptable.

### Demo

**Example with a HIGH critic threshold** (a higher threshold means the agent must achieve a higher score to pass, triggering more refinement cycles):
...
Iterative refinement is configured via the Settings UI:
...
When iterative refinement is active, a visual indicator appears in the status bar so you can see that the agent is in a refinement cycle. The indicator shows the current refinement iteration (e.g., "Refining 1/3"):

![Refinement status indicator in the status bar](https://github.com/user-attachments/assets/7468dd51-a759-49e6-b772-4532dfcd3361)
