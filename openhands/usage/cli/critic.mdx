...
Refinement can also be triggered by specific detected issues (e.g., insufficient testing, missing error handling). When any issue has a probability above the **issue threshold** (default: 75%), refinement is triggered even if the overall score is acceptable.

### Demo

**Example with a higher refinement threshold** — the agent must achieve a higher score to pass, triggering more refinement cycles:
...
**Example with a moderate refinement threshold:**
