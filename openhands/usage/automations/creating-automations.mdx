...
- "Create a GitHub issue with the findings" (requires connected GitHub account)
- "Send a message via the configured notification service"

<Note>
The automation can only send results to services you've configured. Make sure you have the necessary [integrations](/openhands/usage/settings/integrations-settings) or [MCP servers](/openhands/usage/settings/mcp-settings) set up before creating automations that depend on them.
</Note>
...
For more complex automations, you can include plugins that provide additional skills or capabilities. Ask for it like:
...
Plugins are loaded from the [OpenHands extensions repository](https://github.com/OpenHands/extensions) or custom sources you specify.
