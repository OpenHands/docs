...
This is equivalent to using the profile selector button and works without leaving the chat. For a complete workflow with
Agent Canvas screenshots, see the [`/model` Command guide](/openhands/usage/settings/model-command).
...
See the [Model Selection Tool guide](/openhands/usage/settings/model-selection-tool) for setup guidance, an example prompt,
and Agent Canvas screenshots.
