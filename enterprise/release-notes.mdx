...
This release introduces **daily conversation quotas** with a structured 429 response and a direct link to the settings page, and adds **organization-scoped secrets** — a new backend model plus a UI share checkbox with permission-gated actions so admins can share secrets across an org. Users now see **their own budget and usage** in the app, and conversations gain **tags** on start/update along with a `tags__contains` search filter. OAuth MCP installs are now handled from the app server, and the frontend ships an **OpenHands-Neo settings theme** aligned with Agent Canvas. Automation lets admins **pick an agent profile for delegated work** and start automations disabled, and operators can now **configure the Keycloak admin login through Replicated**.
