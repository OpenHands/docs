...
<Tabs>
  ...
  <Tab title="Manual VM Setup">
    ...
    **Obtain a TLS certificate signed by a well-known certificate authority (CA) such as Let's Encrypt**, with SANs
    ...
    (`.pem` or `.crt`) and private key (`.pem` or `.key`) to the VM.

    <Warning>
      A certificate signed by a well-known public CA (such as Let's Encrypt) is strongly
      recommended, since it works without any additional trust configuration. Self-signed
      certificates are supported as an escape hatch (for example, while you're waiting on a
      certificate from your IT team or from a public CA), but they require extra configuration
      (see [Certificate Configuration](#certificate-configuration) below) and add operational
      friction such as browser warnings. Non-internet-routable domains (e.g., `*.internal`,
      `*.lan`) are discouraged because they can be problematic for the DNS resolvers used by
      agent sandboxes; use a routable domain when you have the choice.
    </Warning>
...
- Optionally upload the **Additional Trusted CA Certificates** file (root CA in PEM format)

The **Additional Trusted CA Certificates** field configures cluster-wide trust for
certificates that are not signed by a well-known public CA. Upload a PEM file here if
either of these applies:

- **Your TLS certificate is self-signed or signed by an internal/private CA.** Upload the
  issuing CA (or the self-signed certificate itself if it has no separate issuer) so
  OpenHands components can validate it for traffic inside the cluster.
- **OpenHands integrates with services outside the cluster that use non-public certificates**,
  for example a self-hosted GitLab, Bitbucket Data Center, or Jira Data Center, or a corporate
  TLS-inspecting proxy. Upload the CA(s) that signed those endpoints.

You can concatenate multiple PEM blocks (`-----BEGIN CERTIFICATE-----` ... `-----END CERTIFICATE-----`)
into a single file. The bundle is merged with the standard well-known CAs and mounted into
OpenHands' core components (the web/API, integrations service, MCP service, runtime API, LLM
proxy, and Keycloak) and into the agent runtime sandboxes themselves, so `git`, `curl`, and
other shell commands the agent runs inside a sandbox will also trust it.

<Warning>
  **Self-signed certificates are an escape hatch, not a recommendation.** Uploading a CA
  here makes OpenHands trust the certificate *inside the cluster only*. It does **not**
  make your web browser trust the cert at `app.<your-domain>`. For that, distribute your
  CA to client machines or use a publicly trusted certificate. Where possible, use a
  certificate from a CA that's already in the default trust store (typically a well-known
  public CA).
</Warning>
