...
The install guide provides four commands to run on your VM. SSH into your VM and execute them in order:
...
4. **Install** -- we recommend providing your TLS certificates during installation:
   ...
   ```
   If you used the Terraform module, the certificates are already on the VM. If you provisioned
   manually and have your own certificates on the VM, you can pass them the same way. You can
   also omit the `--tls-cert` and `--tls-key` flags and upload certificates later through the
   Admin Console.
...
You should now see the application configuration page. The left sidebar shows the sections you
need to configure.
