...
<Tabs>
  ...
  <Tab title="Manual VM Setup">
    ...
    **Certificate requirements:** Obtain a TLS certificate signed by a well-known certificate authority (CA)
    such as Let's Encrypt, then copy the certificate (`.pem` or `.crt`) and private key (`.pem` or `.key`)
    to the VM. Self-signed certificates are not supported for the OpenHands application.
...
| `*.runtime.<domain>` DNS + cert SAN | Runtime sandboxes are addressed by dynamic runtime-specific hostnames (subdomain mode only) |
| `runtime.<domain>` DNS + cert SAN | Runtime base hostname for path-based sandbox routing (path-based mode only) |
