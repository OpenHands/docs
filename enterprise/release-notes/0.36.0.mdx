...
- **Agent Canvas Now Available** — The Agent Canvas experience is now accessible at `/canvas` and will coexist with the current Enterprise conversation interface
- **Bitbucket Data Center Support** — Added as a supported Git provider for Skills marketplace registrations
- **Security & Performance** — Improved database-pool resiliency, LLM usage-metrics accuracy, and runtime cleanup performance
