...
The guarantees above isolate the sandbox from the host and from other tenants. Controlling where a
sandbox can reach on *your* network is a separate concern that you own. Because each sandbox is an
ordinary Kubernetes pod, you can apply your normal cluster controls to it—for example, a
[`NetworkPolicy`](https://kubernetes.io/docs/concepts/services-networking/network-policies/) (or your
CNI's equivalent) to restrict egress, or node/VPC-level firewall rules to block access to internal
services and cloud metadata endpoints. If your environment has specific egress requirements, apply
those policies to the sandbox namespace as you would for any other workload.
