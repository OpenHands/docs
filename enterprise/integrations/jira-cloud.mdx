...
2. Log in as the service account and create an API token at
   **id.atlassian.com → Security → API tokens**. Save the token for the
...
- **On older releases, only the typed `@openhands` triggers**: releases
  before the picker-mention fix ignore mentions inserted by Jira's
  autocomplete. Type the literal text and dismiss the autocomplete popup with
  Escape, or upgrade.
