...
- OpenHands Enterprise **0.63.0 or later**. The first Stable-channel release with this behavior is **0.64.0**.
...
The Runtime API's admin endpoints authenticate with an admin password. A fresh installation of OpenHands
Enterprise 0.63.0 or later generates a durable password and stores it in the `admin-password` secret. An
installation upgraded from an earlier release may still have an empty value. The helper script in Step 2
reads the secret and reports an error before a save or delete when the value is empty.

To initialize an empty password or choose your own:
