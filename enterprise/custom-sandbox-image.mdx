...
<Tabs>
  <Tab title="VM Install (Admin Console)">
    ...
    configs layer on top of the installer's ConfigMap entries rather than replacing them: the
    installer's default `v1_current` pool keeps running while you add new custom entries alongside
    it. Saving an API config with the same name as a ConfigMap entry overrides that entry;
    deleting it reverts to the ConfigMap value.
  </Tab>
  <Tab title="Kubernetes (Helm)">
    <Warning>
      **The first configuration you save takes over warm pool management.** While the Runtime API
      database holds *any* configurations, the installer-managed default pool is ignored entirely,
      and Admin Console image settings stop affecting warm pools. Always re-declare the default
      image as one of your configurations (Step 3 below does this). To hand control back to the
      installer, delete **all** configurations (see
      [Reverting](#reverting-to-installer-managed-configuration)).

      To enable overlay mode instead (so API configs layer on top of the installer pool rather
      than replacing it, matching the Replicated behavior), add
      `runtime-api.env.WARM_RUNTIME_CONFIG_OVERLAY: "1"` to your `values.yaml` and run
      `helm upgrade`.
    </Warning>
  </Tab>
</Tabs>
...
The Runtime API admin endpoints require an admin password stored in the `admin-password` Kubernetes
secret. The helper script (Step 2) reads this secret automatically, so no manual step is needed
for a standard installation — but the setup differs by install type.

<Tabs>
  <Tab title="VM Install (Admin Console)">
    The password is **auto-generated** at install (`{{repl RandomString 32}}`) and already in
    the secret. The helper script reads it automatically — no action required.

    To set a memorable password, or to rotate the generated one:
    ...
    The Admin Console updates the secret and rolls out the runtime-api automatically. The
    password persists across all future Admin Console deploys.

    <Warning>
      Do not use `kubectl patch` to set the password. The Admin Console manages the
      `admin-password` secret and overwrites it on every deploy, so a patched value is lost the
      next time you save any config change.
    </Warning>
  </Tab>
  <Tab title="Kubernetes (Helm)">
    The password was set when you created the `admin-password` secret during installation:
    ...
    Store the new value in your password manager before running the first command.
  </Tab>
</Tabs>

### Step 2: Save the Helper Script

Save the script below as `warm-runtime-configs.sh`. How it reaches the runtime-api differs by install
type — choose your tab.

<Tabs>
  <Tab title="VM Install (Admin Console)">
    The runtime-api is exposed at `https://runtime-api.<your-base-domain>` with a valid TLS certificate.
    All API calls go directly to that URL from your local machine — no cluster shell or `kubectl exec`
    needed for day-to-day operations. The script reads credentials from Kubernetes secrets on first run;
    export them as environment variables afterward to run the script entirely without cluster access.
    ...
    #
    # Credentials are read from Kubernetes secrets automatically. To run without
    # cluster access, export these variables before calling the script:
    #   export RUNTIME_API_URL=https://runtime-api.<your-base-domain>
    #   export API_KEY=<value from the default-api-key secret>
    #   export ADMIN_PASSWORD=<value from the admin-password secret, or set in Admin Console>
    set -euo pipefail
    ...
    After the first run, print the discovered values and save them somewhere secure so future
    runs need no cluster access at all:

    ```bash
    # Run once from a machine with kubectl access, then save the output
    NAMESPACE=openhands
  ...
  <Tab title="Kubernetes (Helm)">
    The runtime-api ingress is off by default on Helm installs. The script tunnels each API call
    into the runtime-api pod with `kubectl exec`. You need `kubectl` access for every operation.
    ...
    To expose the runtime-api externally on a Helm install, add this to your `values.yaml` and run
    `helm upgrade`:
    ...
    runtime-api:
      ingress:
        ...
        tls: true
        tlsSecretName: runtime-api-tls
    ```

    Once exposed, switch to the **VM Install** script above (set `RUNTIME_API_URL` to your hostname).
  </Tab>
</Tabs>
...
Save `v1_current` as an API-managed config. The application treats the configuration with that name
as the system default when no user has picked a preference:
...
<Note>
  **VM installs (Replicated):** The installer's `v1_current` pool continues running while you
  add API-managed entries — no takeover occurs. Saving `v1_current` here overrides the ConfigMap
  entry so the API manages it explicitly; you can skip this sub-step if you do not need to change
  the default image or want the installer to continue owning that entry.

  **Helm installs:** This step is required. It re-declares the default pool so it survives the
  takeover described above.
</Note>
...
| `fuse_s3_mount` | boolean | No | When `true`, the sandbox mounts `/workspace` from object storage via FUSE rather than a PVC. Requires the FUSE device plugin and `FUSEY_*` env vars on runtime-api. Leave unset (`false`) for standard PVC-backed sandboxes. |
...
Delete **all** API-managed configurations. The Runtime API then uses only the installer's ConfigMap
entries on the next reconciler cycle:
...
<Note>
  **VM installs (Replicated):** The installer's `v1_current` pool was never replaced — overlay mode
  kept it running throughout. Deleting all API-managed configs simply lifts any overrides, and the
  ConfigMap entries continue as the sole source.

  **Helm installs:** Deleting all API-managed configs hands warm pool management back to the
  installer's ConfigMap. The Admin Console image settings resume affecting the pool.
</Note>
...
|---|---|
| `HTTP 403: Admin functionality is disabled` | The runtime-api deployment has no admin password configured (non-standard install). See Step 1 for your install type. |
| `HTTP 401` on login | Wrong password, or the challenge expired (challenges are single-use and expire after 5 minutes; the script fetches a fresh one per call). To check the current password: `kubectl get secret admin-password -n openhands -o jsonpath='{.data.admin-password}' \| base64 -d`. On Replicated installs, the password only changes if you update it in the Admin Console and deploy. |
...
| Saved a config but the dropdown does not show it | The app server caches the config list for 60 seconds; the UI caches it for up to 5 minutes. Wait up to 5 minutes, or navigate away from and back to the Settings page to prompt a fresh fetch. Confirm the config was saved with `./warm-runtime-configs.sh list`. |
...
Returns `200` with `{"configs": [{name, image, working_dir, command, environment, count,
run_as_user, run_as_group, fs_group, fuse_s3_mount, source, ...}, ...]}`.

The `source` field is `'db'` for API-managed entries and `'file'` for ConfigMap entries. On
Replicated installs (overlay mode), the list is the full effective set — ConfigMap entries merged
with same-named API entries overriding them. On Helm installs (legacy mode, default), only API-saved
entries are returned while the database contains any rows; the installer's ConfigMap entries do not
appear in the list.
