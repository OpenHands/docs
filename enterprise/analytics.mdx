  ...
  <Tab title="Kubernetes (Helm)">
    ...
    laminar:
      ...
      frontend:
        ...
        # Authenticate the Laminar UI through the same Keycloak realm as
        # OpenHands Enterprise. The chart-managed `keycloak-realm` Secret
        # supplies the client credentials; the issuer points at the `allhands`
        # realm on your auth hostname. nextauthUrl/nextPublicUrl must match the
        # frontend hostname above so the OAuth callback is built correctly.
        env:
        ...
        extraEnv:
          ...
          - name: AUTH_KEYCLOAK_ISSUER
            value: "https://auth.<your-base-domain>/realms/allhands"
...
<Note>
  For the **Continue with Keycloak** flow to succeed, the Keycloak client referenced by the `keycloak-realm` Secret must allow the Laminar callback. Its **Valid Redirect URIs** must include `https://analytics.<your-base-domain>/api/auth/callback/keycloak` and its **Web Origins** must include `https://analytics.<your-base-domain>`.

  These must match the hostname in `laminar.frontend.ingress.hostname`. If your DNS uses the simple (single-level) layout — for example `analytics.<your-base-domain>` rather than a nested `analytics.app.<your-base-domain>` — confirm the redirect URI matches that hostname exactly, otherwise Keycloak rejects the login with an `Invalid parameter: redirect_uri` error.
</Note>
