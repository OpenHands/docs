...
<Tabs>
  <Tab title="VM Install (Admin Console)">
    ...
    Enter the URI of the plugin catalog you want to load. To use the official OpenHands plugin
    catalog, enter:

    ```text
    github://OpenHands/plugin-directory
    ```

    To pin to a specific release of the catalog, append a `@ref` tag:

    ```text
    github://OpenHands/plugin-directory@v1.0.0
    ```
  ...
  <Tab title="Kubernetes (Helm)">
    ...
    plugin-directory:
      ...
      appEnv:
        ...
        MARKETPLACE_SOURCE: "github://OpenHands/plugin-directory"
...
|--------|---------|-------|
| `github://owner/repo` | `github://OpenHands/plugin-directory` | Loads from the default branch of the repository |
| `github://owner/repo@ref` | `github://OpenHands/plugin-directory@v1.2.0` | Loads from a specific branch, tag, or commit SHA |
| `https://example.com/catalog.json` | `https://cdn.example.com/plugins/catalog.json` | Loads a catalog JSON file over HTTPS |

Use the official `github://OpenHands/plugin-directory` catalog to access the community plugin
directory maintained by the OpenHands team.
