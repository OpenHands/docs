...
We recommend running agent sandboxes on their own node pool, separate from the OpenHands application
services. This keeps the untrusted sandbox workload isolated, and lets the sandbox pool scale on its
own, since sandboxes are created and torn down far more frequently than the services.

- **General**: the OpenHands application services and cluster add-ons. Standard EKS nodes on the
  Amazon Linux 2023 AMI; on-demand or Spot.
...
work). Size the sandbox pool by peak concurrent sessions, and configure it so a node is only removed
