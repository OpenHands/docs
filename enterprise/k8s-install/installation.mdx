...
- A **GitHub App or GitLab OAuth application** for user authentication. No
  GitHub App yet? Our
...
Then create the secret for your authentication provider:

<Tabs>
  <Tab title="GitHub">
    If you don't have a GitHub App yet, run our
    [create_github_app script](https://github.com/All-Hands-AI/OpenHands-Cloud/tree/main/scripts/create_github_app)
  ...
  <Tab title="GitLab">
    ```bash
    kubectl -n openhands create secret generic gitlab-app \
      --from-literal=client-id=<gitlab-oauth-application-id> \
      --from-literal=client-secret=<gitlab-oauth-application-secret>
    ```
  </Tab>
</Tabs>
...
# Enable the authentication provider matching the secret you created in Step 2
github:
  enabled: true
# gitlab:
#   enabled: true
#   host: gitlab.example.com   # only for self-hosted GitLab
...
<Warning>
  Without an authentication provider (`github` or `gitlab`) users cannot log in,
  and without the `sandbox`/runtime configuration agents cannot start. These
  sections are not optional. The bundled MinIO ships with development
