...
- DNS records you control, following the `app.<base-domain>` layout used
  ...
  cluster's ingress; see the "DNS and TLS Setup" section of the
  [Quick Start](/enterprise/quick-start) for the full record reference.
- **TLS certificates covering all of the hostnames above** — via cert-manager or
  your own certificates. The application generates `https://` URLs throughout,
  and in-cluster components verify the certificates.
