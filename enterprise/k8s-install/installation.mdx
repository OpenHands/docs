...
- DNS records you control, following the `app.<base-domain>` layout used
  ...
  `*.runtime.openhands.example.com` for runtime sandboxes
- **Publicly-trusted TLS certificates covering all of the hostnames above** —
  via cert-manager or your own certificates. This is not optional: the
  application generates `https://` URLs throughout, and in-cluster components
  verify the certificates, so self-signed certificates break login and agent
  communication. Wildcard certificates require a DNS-01-capable issuer.
- An **authentication method** for user login — GitLab, Bitbucket Data Center,
  and more are supported; this guide uses a **GitHub App**. No GitHub App yet?
  Our [script](/enterprise/quick-start#create-a-github-app) creates one
...
<Note>
  The example above uses cert-manager to issue certificates (`tls.enabled: true`
  with an ACME `email`). If instead you terminate TLS at your ingress controller
  with your **own certificates**, disable the chart's certificate handling and
  install the certificate at the ingress yourself:
  ...
  The certificates must still cover all of the hostnames listed in the
  Prerequisites, and must be publicly trusted.
</Note>
...
<Note>
  The URL above installs from the **Stable** channel. If your license is
  assigned to a different channel, include that channel's slug in the path —
  otherwise the pull fails with `denied: Channel not found for license`:

  ```bash
  helm install openhands oci://registry.replicated.com/openhands/<channel-slug>/openhands \
    --namespace openhands \
    --values values.yaml
  ```
</Note>
...
The CLI prints `Successfully uploaded support bundle` without a link — find the
uploaded bundle as the most recent entry in the Vendor Portal.
...
| `helm registry login` or chart pull returns 401/403 | License credentials incorrect, the license isn't enabled for Helm installs, or you're pulling from a channel your license isn't assigned to (add the channel slug to the URL, see Step 4) — contact support if unsure |
...
| Uploaded support bundle shows "Instance ID: Not set" or "No app-info file" in the Vendor Portal | Known limitation of the bundled Replicated SDK's collectors — the bundle still uploads and authenticates, only instance-event correlation is affected. Include the timestamp when contacting support |
