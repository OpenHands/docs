...
storage, and your LLM provider. PostgreSQL and Valkey, a Redis-compatible
cache, run embedded in the cluster; the bundled PostgreSQL needs a database
name and database creation turned on, both shown below (to use your own
database instead, see [External PostgreSQL](/enterprise/external-postgres)).
