...
### Estimating cluster requirements

Calculate your cluster capacity based on expected concurrent usage:

```
Total memory needed = 
  (OpenHands server memory × replicas) +
  (Runtime API memory × replicas) +
  (Other components memory) +
  (Sandbox memory × max concurrent sandboxes)
```

**Example**: 10 concurrent sandboxes with 8Gi each, 2 OpenHands replicas:

```
= (4Gi × 2) + (1.5Gi × 1) + (~4Gi other) + (8Gi × 10)
= 8Gi + 1.5Gi + 4Gi + 80Gi
= ~94Gi total cluster memory
```

### Node sizing recommendations

| Concurrent Sandboxes | Recommended Node Memory | Recommended Nodes |
|---------------------|------------------------|-------------------|
| 1-5 | 32Gi | 1-2 |
| 5-20 | 64Gi | 2-4 |
| 20-50 | 128Gi | 4-8 |
...
<Note>
  These are estimates. Actual requirements depend on your specific workloads,
  agent usage patterns, and configured resource limits.
</Note>
