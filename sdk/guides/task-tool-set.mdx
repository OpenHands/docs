...
## Background DelegateExecutor

`TaskToolSet` remains blocking and supports resuming a completed task. Use the
lower-level `DelegateExecutor` when a parent must continue working while one or
more named sub-agents run in the background.

Set `background=True` on a `delegate` action. The observation returns one stable
task ID for each sub-agent in the `tasks` map. Use lifecycle actions to check the
status, retrieve output, or stop the task.
...
# Assume conversation already exists and "code_reviewer" is registered as
...
started = delegate(
    DelegateAction(
        command="delegate",
        tasks={"reviewer": "Review the authentication changes."},
        background=True,
    ),
    conversation,
)
assert started.task_ids is not None
task_id = started.task_ids["reviewer"]

status = delegate(
    DelegateAction(command="status", task_id=task_id),
    conversation,
)
if status.status == DelegateTaskStatus.COMPLETED:
    output = delegate(
    ...
    DelegateTaskStatus.QUEUED,
    DelegateTaskStatus.RUNNING,
}:
    # Request cooperative cancellation when the result is no longer needed.
    delegate(
...
Background tasks move through `queued`, `running`, `completed`, `failed`, or
`cancelled`. Multiple sub-agents can run at the same time, but a second task for
the same spawned agent is rejected until its active task reaches a terminal
state. Reading status or output more than once is safe and does not duplicate
usage metrics.

<Warning>
Background task IDs are in-process control handles owned by one
`DelegateExecutor` and parent conversation. The registry is not restored after a
process restart. Persisted sub-agent conversation data does not restore running
workers or their task IDs.
</Warning>
