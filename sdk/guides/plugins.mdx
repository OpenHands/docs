...
The SDK provides utilities to install plugins to a local directory (`~/.openhands/plugins/installed/` by default). Installed plugins are tracked in `.installed.json`, which also stores the persistent enabled flag. `list_installed_plugins` returns all tracked plugins, while `load_installed_plugins` loads only those marked as enabled. Use `enable_plugin` and `disable_plugin` to toggle a plugin without uninstalling it.
...
# Disable a plugin (load_installed_plugins only returns enabled ones)
