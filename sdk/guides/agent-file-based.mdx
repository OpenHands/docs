...
When the same agent name is defined in multiple places, the highest-priority source wins:

1. **Programmatic** `register_agent()` calls (never overwritten)
2. **Plugin agents** (`Plugin.agents`)
3. **Project-level** file-based agents
4. **User-level** file-based agents
