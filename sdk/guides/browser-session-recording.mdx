...
Recording files are written under `.agent_tmp/browser_observations/recording-<timestamp>/` relative to the browser tool process working directory. Events are flushed periodically, and stopping the recording saves any remaining events. This location is separate from the conversation's `persistence_dir`.

For browser setup and tool configuration, see [Browser Use](/sdk/guides/agent-browser-use).
