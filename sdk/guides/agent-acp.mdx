...
`structured_output` is the programmatic, provider-neutral configuration for ACP structured output. `StructuredOutputConfig` and `StructuredOutputMode` are public exports from `openhands.sdk`. Create a `StructuredOutputConfig` with a caller-supplied JSON Schema. `StructuredOutputMode` is currently limited to `"json_schema"`.
...
# Or set it on ACPAgentSettings and create the agent from the settings.
settings = ACPAgentSettings(
    acp_server="claude-code",
    structured_output=structured_output,
)
settings_agent = settings.create_agent()
```

The configuration is intentionally bounded: it accepts the caller's schema, not an arbitrary raw ACP `_meta` dictionary. The caller owns the schema's canonical semantics and must validate the returned semantic result. The SDK only handles provider and ACP attachment mechanics; it does not interpret the schema's domain fields.

The only currently qualified mapping is Claude ACP. On that path, the SDK attaches the schema to Claude's provider-native JSON Schema output mechanism during session setup, including resumed sessions. Any provider-specific session metadata used for that mapping is Claude-specific and is **not** an ACP standard field.

If structured output is requested for a provider without a qualified mapping, the SDK fails explicitly before ACP session dispatch. It does not silently ignore the configuration or downgrade to prompt-only behavior. Provider support is provider-specific and must be explicitly implemented and qualified.
