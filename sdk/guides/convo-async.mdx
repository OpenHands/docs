...
You can interrupt and cancel an async conversation while it is running — even mid-LLM-generation or during a long-running tool call. Call `interrupt()` from any thread or coroutine; the conversation transitions to `PAUSED` and can be resumed later with another `arun()` / `run()`.
