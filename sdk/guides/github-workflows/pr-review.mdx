...
Automatically review pull requests, providing feedback on code quality, security, and best practices. Reviews are triggered by adding the `review-this` label to the PR.
...
#  4. Trigger the review by adding the "review-this" label to any PR
name: PR Review by OpenHands

on:
    # Trigger when the "review-this" label is added
    pull_request:
        types: [labeled]
...
jobs:
    pr-review:
        # Run when review-this label is added
...
- Users with write access (maintainers) can trigger reviews by adding the `review-this` label.
