...
<Note>
Using a unique name like `custom-codereview-guide` allows BOTH your custom skill AND the default `code-review` skill to be triggered by `/codereview`. The agent receives guidelines from both skills.

If you name your file `code-review.md` with `name: code-review`, it will completely **override** the default public skill instead of supplementing it.
</Note>
