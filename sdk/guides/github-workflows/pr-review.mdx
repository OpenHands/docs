...
<Note>
To use the "request reviewer" trigger, the specified account must already have access to your repository. GitHub only allows requesting reviews from collaborators or org members/teams with repo access. The `openhands-agent` handle here is a placeholder—either change it to a user/team you control that has access, or use the `review-this` label trigger, which works without adding an account.
</Note>
