...
The PR review agent uses skills from the [OpenHands/extensions](https://github.com/OpenHands/extensions) repository by default. You can customize behavior in two ways:

1. **Replace the default skill**: Use the same name (`code-review`) to completely replace the default skill with your custom guidelines
2. **Add supplementary guidelines**: Use a different name to add your project-specific guidelines alongside the default skill

### Option 1: Replace the Default Skill

Create `.agents/skills/code-review.md` in your repository with `name: code-review` to completely replace the default public skill:
...
### Option 2: Add Supplementary Guidelines (Recommended)

To keep the default review skill AND add your project-specific guidelines, use a **different name**. Both skills will be triggered by `/codereview`:
...
<Tip>
Using a unique name like `my-project-review` allows BOTH your custom skill AND the default `code-review` skill to be triggered by `/codereview`. The agent receives guidelines from both skills.
</Tip>
