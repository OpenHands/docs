...
## Budget Denials and Rate Limits

A LiteLLM HTTP 429 budget denial is surfaced as `LLMRateLimitError` immediately. The SDK recognizes `budget_exceeded` and `Budget has been exceeded` and skips both retry backoff and model fallback. Increase the applicable budget or wait for its reset before retrying. Temporary rate limits still use the configured retries.
