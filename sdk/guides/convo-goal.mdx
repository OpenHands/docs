...
```python icon="python" focus={2,5-7,11}
...
`GoalController` owns the continue-vs-stop decision logic and the iteration cap. It performs **no I/O**: a driver owns sending messages and running the agent.
...
`judge_goal` is the reusable kernel: a pure `(objective, transcript) → GoalVerdict` evaluator with no dependency on the loop. Use it directly to build a `/status` command, a stop hook, or a server endpoint:
