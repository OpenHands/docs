...
<Note>
This example is available on GitHub: [examples/01_standalone_sdk/25_agent_delegation.py](https://github.com/OpenHands/agent-sdk/blob/main/examples/01_standalone_sdk/25_agent_delegation.py)
</Note>
...
Before delegating work, you must first spawn sub-agents with meaningful identifiers:
...
- Gets a unique identifier that you specify (e.g., "lodging", "activities")
...
Once sub-agents are spawned, you can delegate tasks to them:
...
You can limit the maximum number of concurrent sub-agents:
...
**Returns:**
- `success`: Whether spawning succeeded
- `spawned_ids`: List of successfully spawned agent IDs
...
**Returns:**
- `success`: Whether all sub-agents completed successfully
- `results`: List of results from all sub-agents (one per agent)
- `message`: Summary of the delegation operation
