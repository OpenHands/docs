...
description: Execute multiple tools concurrently within a single LLM response to improve throughput for independent operations.
---
...
<Warning>
**Experimental Feature**: Parallel tool execution is still experimental. By default, `tool_concurrency_limit` is set to `1` (sequential execution). Increasing this value may improve performance for independent operations, but use at your own risk—concurrent execution can lead to race conditions or unexpected behavior for tools that share state.
</Warning>
...
```mermaid
flowchart LR
    LLM["LLM Response"] --> TC1["Tool Call 1"]
    LLM --> TC2["Tool Call 2"]
    LLM --> TC3["Tool Call 3"]
    ...
    subgraph Parallel["Parallel (tool_concurrency_limit > 1)"]
        P1["Tool 1"]
        P2["Tool 2"]
        P3["Tool 3"]
    end
...
With sequential execution (default), tools run one after another. With parallel execution enabled, independent tools run concurrently up to the specified limit.
...
agent = Agent(
    ...
    tool_concurrency_limit=4,  # Execute up to 4 tools concurrently
)
```
...
| `>8` | High parallelism—use with caution |
...
1. **Start conservative**: Begin with `tool_concurrency_limit=1` and increase gradually
2. **Test thoroughly**: Verify your workflow behaves correctly with concurrent execution
3. **Use for read-heavy workloads**: Parallel execution works best for independent read operations
4. **Monitor for race conditions**: Watch for unexpected behavior when tools share state
...
<Note>
This example is available on GitHub: [examples/01_standalone_sdk/45_parallel_tool_execution.py](https://github.com/OpenHands/software-agent-sdk/blob/parallel-tool-execution/examples/01_standalone_sdk/45_parallel_tool_execution.py)
</Note>
