...
falls back to alternative LLM models when the first model fails.

The FallbackRouter will:
1. Try the first model in the list
2. If it fails (rate limit, connection error, etc.), try the next model
3. If that fails, try the next model, and so on
...
# Create LLMs in order of preference
primary_llm = LLM(
...
# The router will try models in the order they appear in the list
fallback_router = FallbackRouter(
    usage_id="fallback-router",
    llms=[primary_llm, fallback1_llm, fallback2_llm],
)
...
1. **Trying the first model**: All requests start with the first model in the list
2. **Catching errors**: If the first model fails with errors like rate limits, connection issues, or service unavailability, the router catches the exception
3. **Falling back**: The router tries each subsequent model in order until one succeeds
...
- **Cost Optimization**: Use expensive models first and cheaper models as fallbacks
...
# Models are tried in list order
fallback_router = FallbackRouter(
...
1. **Order models by preference**: Put your preferred model first in the list and less preferred models later
2. **Monitor logs**: Enable logging to track failover events and identify reliability issues
3. **Consider costs**: Use expensive models first and cheaper alternatives as fallbacks
