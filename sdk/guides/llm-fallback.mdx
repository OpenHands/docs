...
4. If all fallbacks fail, the original primary error is raised, unless a fallback reports a budget denial
...
<Warning>
Transient rate limits, timeouts, and connection errors can trigger fallback after retries.
Provider quota errors such as `insufficient_quota` and `usage_limit_reached` skip retries and can fall back immediately.

Explicit LiteLLM budget denials (`budget_exceeded` or `Budget has been exceeded`) stop immediately, without retrying or trying another model. This also applies when a fallback reports the denial: its budget error is raised and later fallbacks are not attempted. Raise the applicable limit or wait for its reset before retrying.
</Warning>
...
For eligible failures, `fallback-1` is tried first, then `fallback-2`. If both fail, the primary error is raised. A budget denial stops the chain immediately.
