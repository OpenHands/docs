...
<Warning>
Enable/disable APIs for installed skills (including the `enabled` metadata flag) are
available in the upcoming SDK release tied to https://github.com/OpenHands/software-agent-sdk/pull/2322.
If you are using an earlier SDK version, omit `enable_skill` / `disable_skill` and
note that `.installed.json` will not include the `enabled` field.
</Warning>
