...
**Multiple Skills with Same Trigger**: Skills with different names but the same trigger can coexist and will ALL be activated when the trigger matches. This allows you to:

- **Replace a skill**: Use the same name as a public skill to completely replace it
- **Supplement a skill**: Use a different name with the same trigger to add your guidelines alongside the public skill
...
agent_context = AgentContext(
    ...
    skills=[
        Skill(
            name="my-project-review",  # Different name = coexists
...
<Tip>
To add project-specific guidelines alongside public skills, use a unique name like `my-project-review` instead of `code-review`. Both skills will be triggered together.
</Tip>
