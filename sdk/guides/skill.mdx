...
Skills modify agent behavior by injecting additional context and rules. You can define skills inline in code or load them from disk following the [AgentSkills standard](https://github.com/OpenHands/skills).
...
- **Always-active skills** (`trigger=None`): Full content is included directly in the system prompt
- **Triggered skills** (`KeywordTrigger`, `TaskTrigger`): Listed in a `<SKILLS>` section so the agent knows they exist, with full content injected only when triggered

This design keeps the system prompt concise while letting the agent proactively look up skill content by reading the skill's location path.

For triggered skills, the system prompt includes:
...
When a keyword is detected in a user message, the skill's full content is injected into the conversation.
...
The OpenHands SDK supports skills in the [AgentSkills standard](https://agentskills.io/home), which is compatible with Claude's skill format. This allows you to organize skills as self-contained packages with their own scripts, references, and assets.
...
<Note>
The `triggers` field is optional. If omitted, the skill is always active and its full content is included in the system prompt. If `triggers` is defined, the skill is listed in the `<SKILLS>` section and its content is only injected when a trigger keyword is detected.
</Note>
...
Skills with triggers are listed in the system prompt with their location path (see [How Triggered Skills Work](#how-triggered-skills-work)). When a skill is triggered by a keyword match, the agent receives the full skill content:
...
Here's a complete skill demonstrating the AgentSkills standard:
