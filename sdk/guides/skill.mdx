...
Skills modify agent behavior by injecting additional context and rules. OpenHands supports the [AgentSkills standard](https://agentskills.io/specification) with extensions for backward compatibility with legacy skill formats.

## Quick Reference: Skill Behavior

OpenHands handles skills differently based on their **format** and **trigger configuration**:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         SKILL CATEGORIZATION FLOW                           │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Is it a SKILL.md file?                                                     │
│       │                                                                     │
│       ├── YES (AgentSkills format) ──► Listed in <available_skills>         │
│       │       │                         • Name, description, location only  │
│       │       │                         • Agent reads full content on demand│
│       │       │                                                             │
│       │       └── Has triggers? ──► ALSO auto-inject when triggered         │
│       │                                                                     │
│       └── NO (Legacy OpenHands format)                                      │
│               │                                                             │
│               ├── Has triggers? ──► Listed in <available_skills>            │
│               │                      • Content injected when triggered      │
│               │                                                             │
│               └── No triggers ──► Full content in <REPO_CONTEXT>            │
│                                   • Always active in system prompt          │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

| Skill Type | Format | Trigger | System Prompt | Content Loading |
|------------|--------|---------|---------------|-----------------|
| AgentSkills (SKILL.md) | `is_agentskills_format=True` | None | `<available_skills>` | Agent reads file on demand |
| AgentSkills (SKILL.md) | `is_agentskills_format=True` | Has triggers | `<available_skills>` | Auto-inject on trigger + agent can read anytime |
| Legacy with triggers | `is_agentskills_format=False` | `KeywordTrigger`/`TaskTrigger` | `<available_skills>` | Injected when triggered |
| Legacy always-active | `is_agentskills_format=False` | `None` | `<REPO_CONTEXT>` | Full content at startup |

<Note>
**AgentSkills Standard Compliance**: SKILL.md files follow the [progressive disclosure model](https://agentskills.io/specification#progressive-disclosure) - they are always listed with name/description/location. Skills with triggers get the best of both worlds: automatic injection when triggered, plus the agent can proactively read them anytime.
</Note>
...
Skills appear in different sections based on their format and trigger:

**`<available_skills>` section** (progressive disclosure):
- All AgentSkills-format skills (SKILL.md files)
- Legacy skills with triggers (`KeywordTrigger` or `TaskTrigger`)

**`<REPO_CONTEXT>` section** (full content):
- Legacy skills without triggers (`trigger=None`)

```xml
<!-- Skills listed for progressive disclosure -->
<SKILLS>
...
<available_skills>
  ...
  <skill>
    ...
    <location>/path/to/magic-word.md</location>
  </skill>
</available_skills>
</SKILLS>

<!-- Legacy always-active skills have full content here -->
<REPO_CONTEXT>
[BEGIN context from [repo-rules]]
Full content of legacy skill without triggers...
[END Context]
</REPO_CONTEXT>
```
...
#### SKILL.md Behavior (Progressive Disclosure)

All SKILL.md files follow the AgentSkills progressive disclosure model - they are **always listed** in `<available_skills>`:

| Has `triggers`? | System Prompt | Content Loading |
|-----------------|---------------|-----------------|
| Yes | Listed in `<available_skills>` | **Both**: Auto-injected when triggered AND agent can read file anytime |
| No | Listed in `<available_skills>` | Agent reads file on demand |

<Note>
**Best of Both Worlds**: SKILL.md files with triggers get automatic content injection when keywords are detected, while still being listed in `<available_skills>` so the agent can proactively access them even without a trigger match.
</Note>
