...
### How Triggered Skills Work

Skills with triggers (like `KeywordTrigger`) are automatically listed in the agent's system prompt. This allows the agent to:

1. **Know what skills are available** before they're triggered
2. **Proactively look up skill content** by reading the skill's location path

When you configure triggered skills, the system prompt includes a `<SKILLS>` section:
