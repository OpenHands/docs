...
Skills add context and rules to your agent's behavior. OpenHands supports an **extended version** of the [AgentSkills standard](https://agentskills.io/specification) with optional keyword triggers.

## Two Types of Skills

<CardGroup cols={2}>
  <Card title="Always-Active Skills" icon="circle-check">
    Content is **always available** to the agent.
    
    **Use for:**
    - Repository guidelines (`repo.md`)
    - Coding standards
    - Project-specific rules
    
    **How it works:** Content appears in the system prompt so the agent always follows these rules.
  </Card>
  <Card title="Keyword-Triggered Skills" icon="bolt">
    Content is **injected when keywords match**.
    
    **Use for:**
    - Specialized tasks (encryption, PDF handling)
    - Domain-specific knowledge
    - Context-dependent instructions
    
    **How it works:** When a user message contains a trigger keyword, the skill content is automatically added to that message.
  </Card>
</CardGroup>

## Quick Start

The simplest way to add skills is inline in your code:
...
agent_context = AgentContext(
    skills=[
        # Always-active: content always in system prompt
        Skill(
            name="repo-guidelines",
            content="Always use type hints in Python code.",
            trigger=None,  # No trigger = always active
        ),
        # Keyword-triggered: content injected when "encrypt" appears
        Skill(
            name="encryption-helper",
            content="Use the encrypt.sh script in /scripts to encrypt messages.",
...
<Steps>
  <Step title="System Prompt Setup">
    When a conversation starts, OpenHands builds the system prompt:
    - **Always-active skills**: Full content added to system prompt
    - **Triggered skills**: Listed by name/description (agent knows they exist)
  </Step>
  <Step title="User Sends Message">
    When the user sends a message, OpenHands checks for keyword matches.
  </Step>
  <Step title="Trigger Matching">
    If a keyword matches (e.g., user says "encrypt this"), the skill's full content is injected into the user message:
    ```xml
    <EXTRA_INFO>
    The following information has been included based on a keyword match for "encrypt".
    Skill location: /path/to/encryption-helper
    
    Use the encrypt.sh script in /scripts to encrypt messages.
    </EXTRA_INFO>
    ```
  </Step>
  <Step title="Agent Responds">
    The agent sees the injected skill content and follows those instructions.
  </Step>
</Steps>

---

## Inline Skills (Code-Based)
...
Define skills directly in your code:
...
<Tip>
Add `triggers` to make your SKILL.md keyword-activated. Without triggers, the skill is listed but the agent must read the file to see its content.
</Tip>
