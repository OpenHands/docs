...
## Serialize Agent Settings Separately

If you want to persist the agent configuration independently from a running conversation, use `AgentSettings`.
It can serialize the LLM, tool list, and condenser settings to JSON, restore them later, and create a new agent
from the restored configuration.

<Note>
This example is available on GitHub: [examples/01_standalone_sdk/45_agent_settings.py](https://github.com/OpenHands/software-agent-sdk/blob/main/examples/01_standalone_sdk/45_agent_settings.py)
</Note>

```python icon="python" expandable examples/01_standalone_sdk/45_agent_settings.py
# This block is auto-synced from the software-agent-sdk repository.
```

<RunExampleCode path_to_script="examples/01_standalone_sdk/45_agent_settings.py"/>
