...
When you set a `persistence_dir`, your conversation will be persisted to a directory containing JSON files that mirror the `trajectory.json` file structure from OpenHands V0. The structure follows the pattern `.conversations/<conversation-id>/`, where each conversation has its own subdirectory.
