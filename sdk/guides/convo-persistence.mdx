...
When you set a `persistence_dir`, your conversation will be persisted to a directory containing JSON files that mirror the `trajectory.json` file structure from OpenHands V0. Each conversation is stored as a separate JSON file named with its conversation ID.
...
Each JSON file contains the serialized conversation state with all events, messages, and metadata - similar to how `trajectory.json` worked in V0. This makes it easy to inspect, debug, or migrate conversation data between systems.
