...
The `agent_state` field provides a flexible dictionary for storing custom runtime state that persists across iterations. This is useful for agents that need to track feature-specific data.
...
conversation.state.agent_state = {
    **conversation.state.agent_state,
    "retry_count": 3,
    "last_checkpoint": "step_5",
}

# Read state
retry_count = conversation.state.agent_state.get("retry_count", 0)
```

<Warning>
**Important**: To trigger auto-save, you must reassign the entire dictionary. In-place modifications (like `state.agent_state["key"] = value`) will NOT trigger persistence.

```python
# ❌ This does NOT trigger auto-save:
conversation.state.agent_state["key"] = "value"

# ✅ This DOES trigger auto-save:
conversation.state.agent_state = {**conversation.state.agent_state, "key": "value"}
```
</Warning>

### Common Use Cases

- **Iteration tracking**: Store retry counts or iteration numbers for iterative refinement workflows
- **Checkpointing**: Save progress markers for long-running tasks
- **Feature flags**: Track which features have been activated during the conversation
- **Custom metrics**: Store agent-specific metrics that aren't covered by the built-in statistics
