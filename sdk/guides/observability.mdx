...
- **[Laminar](https://www.lmnr.ai/)** - AI-focused observability with browser session replay support
...
[Laminar](https://www.lmnr.ai/) provides specialized AI observability features including browser session replays when using browser-use tools:
...
# Optional: Set the protocol (defaults to gRPC)
...
|----------|-------------|---------|
| `LMNR_PROJECT_API_KEY` | Laminar project API key | `lm_xxxxxxxxxxxxxxxx` |
...
- **`http/protobuf`** or **`otlp_http`** - HTTP with protobuf encoding
- **`grpc`** or **`otlp_grpc`** - gRPC with protobuf encoding (default)

## Platform-Specific Configuration

### Laminar Setup

1. Sign up at [lmnr.ai](https://www.lmnr.ai/)
2. Create a project and copy your API key
3. Set the environment variable:

```bash
export LMNR_PROJECT_API_KEY="lm_xxxxxxxxxxxxxxxx"
```
...
Here's a complete example showing tracing in action:
...
# Configure tracing (before creating agent)
os.environ["LMNR_PROJECT_API_KEY"] = "your-laminar-api-key"
...
# Create conversation - tracing is automatic
conversation = Conversation(agent=agent, workspace=".")
conversation.send_message("List the files in the current directory")
conversation.run()

# View traces in your observability platform
# - Visit Laminar dashboard to see agent execution
# - Session ID will be the conversation UUID
# - All spans will be grouped under the conversation session
```

## Best Practices

1. **Set Environment Variables Early**: Configure OTEL variables before importing the SDK to ensure proper initialization

2. **Use Conversation IDs**: The SDK automatically uses conversation UUIDs as session IDs for trace grouping

3. **Monitor Costs**: Combine observability with [metrics tracking](/sdk/guides/metrics) to understand both performance and cost

4. **Filter Noise**: Some backends let you sample traces - configure this at the collector level, not in the SDK

5. **Secure Secrets**: Never commit API keys to version control. Use environment variables or secret management systems

6. **Test Locally**: Use Jaeger for local development before deploying to production observability platforms
