...
description: Cancel agent execution during LLM reasoning or tool execution using async interrupts.
---
...
### Interrupting During Async Execution

Use `conversation.interrupt()` to cancel an agent mid-task — whether it's waiting for an LLM
completion or executing a long-running tool. Combined with `arun()`, this gives you full async
...
The interrupt works in two phases:
1. **During LLM completion** — cancels the in-flight HTTP request to the model provider.
2. **During tool execution** — cancels the running tool (e.g., a long shell command).

After interruption the conversation transitions to `PAUSED` and can be resumed with another
`arun()` or `run()` call.

### Wiring Ctrl-C

You can also map OS signals to `interrupt()` for interactive scripts:

```python icon="python" wrap
loop = asyncio.get_running_loop()
loop.add_signal_handler(
    signal.SIGINT,
    lambda: conversation.interrupt(),
)
```
...
<Note>
This example is available on GitHub: [examples/01_standalone_sdk/50_async_cancellation.py](https://github.com/OpenHands/software-agent-sdk/blob/main/examples/01_standalone_sdk/50_async_cancellation.py)
</Note>

This example asks the agent to write and run a 30-second countdown script, then
interrupts it (via an auto-timer or Ctrl-C) to demonstrate cancellation during both
LLM reasoning and tool execution.

```python icon="python" expandable examples/01_standalone_sdk/50_async_cancellation.py
"""
...
<RunExampleCode path_to_script="examples/01_standalone_sdk/50_async_cancellation.py"/>

## Next Steps

- **[Conversation with Async](/sdk/guides/convo-async)** - Run conversations asynchronously
- **[Pause and Resume](/sdk/guides/convo-pause-and-resume)** - Pause and resume agent execution
- **[Send Message While Processing](/sdk/guides/convo-send-message-while-running)** - Send messages during execution
