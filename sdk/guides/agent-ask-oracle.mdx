...
`ask_oracle` is useful when an agent is stuck, comparing approaches, or wants another model to review a risky decision. The Oracle call is stateless from the Oracle's point of view: it receives the current conversation context plus the agent's question, returns a recommendation, and does not take over the conversation.
