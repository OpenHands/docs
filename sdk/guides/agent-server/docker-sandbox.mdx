...
<Note>The Docker sandbox image ships with the agent-server and its execution tools. Editor experiences are provided by the OpenHands App when available; do not assume a bundled VSCode process, editor port, or editor-specific environment variable in a portable SDK Docker deployment. VNC and other optional services likewise depend on the selected image and deployment configuration.</Note>
...
> **App migration note:** The bundled VS Code sections below describe the legacy editor-capable Docker sandbox example only. A portable SDK Docker image must not assume a VS Code process, editor port, or `/api/vscode/url`; use the OpenHands App editor handoff when available. Keep `extra_ports=True` and this example only when your selected image explicitly provides the legacy editor capability.

#### Legacy VS Code-Enabled DockerWorkspace

The legacy editor-capable workspace can be configured with extra ports for VS Code access:
