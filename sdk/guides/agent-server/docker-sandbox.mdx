...
<Warning>
`DockerWorkspace` publishes its ports on `127.0.0.1` by default, so the Agent Server is reachable only from the Docker host. Each workspace automatically generates a unique session API key, configures the container with it, and reuses the stored key for all workspace connections.

To accept connections from other machines, explicitly set `public=True`:

```python icon="python"
from openhands.workspace import DockerWorkspace

with DockerWorkspace(public=True) as workspace:
    ...
```

Public workspaces use the same generated key to authenticate API requests. Trusted external clients can obtain it from `workspace.api_key`. Use TLS and network access controls whenever exposing the server beyond the local machine.
</Warning>
