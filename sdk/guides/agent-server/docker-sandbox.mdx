...
<Warning>
`DockerWorkspace` publishes its ports on `127.0.0.1` by default, so the Agent Server is reachable only from the Docker host. To accept connections from other machines, configure a session API key before explicitly setting `public=True`:
...
`SESSION_API_KEY` and `OH_SESSION_API_KEYS_0` are forwarded by default, and the SDK client automatically uses the forwarded key. Constructing a public workspace without one of these environment variables raises a validation error. Use TLS and network access controls whenever exposing the server beyond the local machine.
</Warning>
