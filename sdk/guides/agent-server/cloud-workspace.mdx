...
The workspace uses Bearer token authentication as per the OpenHands Cloud API:
...
This key is sent as `Authorization: Bearer <api_key>` header to authenticate your requests to OpenHands Cloud.

### Custom Sandbox Specifications

You can optionally specify a sandbox spec ID for custom environments:

```python highlight={4}
OpenHandsCloudWorkspace(
    cloud_api_url="https://app.all-hands.dev",
    cloud_api_key=cloud_api_key,
    sandbox_spec_id="your-custom-sandbox-spec",
)
```

Sandbox specs allow you to customize the environment with specific tools, dependencies, or configurations.
...
All agent execution happens on the OpenHands Cloud infrastructure.
