...
Under the hood, `get_llm()` calls `GET /api/v1/users/me?expose_secrets=true` with dual authentication (Bearer token + session key) to securely retrieve your LLM credentials.
...
<Note>
This example is available on GitHub: [examples/02_remote_agent_server/10_cloud_workspace_saas_credentials.py](https://github.com/OpenHands/software-agent-sdk/blob/main/examples/02_remote_agent_server/10_cloud_workspace_saas_credentials.py)
</Note>
...
```python icon="python" expandable examples/02_remote_agent_server/10_cloud_workspace_saas_credentials.py
...
cd agent-sdk
uv run python examples/02_remote_agent_server/10_cloud_workspace_saas_credentials.py
```
