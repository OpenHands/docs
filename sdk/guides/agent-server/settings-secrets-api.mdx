...
- Supporting encrypted secrets in transit for defense-in-depth security
...
For frontend clients that need to pass secrets back to start conversations, use the `X-Expose-Secrets` header:
...
This allows frontends to receive encrypted secrets that can be safely passed back when starting a conversation with `secrets_encrypted=True`.
...
2. Verify encrypted secrets mode with `X-Expose-Secrets: encrypted`
3. Run a real agent conversation using the stored settings
