...
<Warning>
**Binary Mode Limitation**: Custom tools only work with **source mode** deployments of the agent server. If you're using a pre-built binary agent server (built with PyInstaller), custom tools cannot be dynamically imported at runtime because the binary's frozen Python interpreter can only load modules that were bundled at build time.

When using `DockerDevWorkspace`, the agent server is built in source mode by default, so custom tools work correctly. However, if you're deploying a standalone binary agent server, you'll need to either:
- Use source mode instead (recommended for custom tools)
- Rebuild the binary with your custom tools included at build time

See [GitHub issue #1531](https://github.com/OpenHands/software-agent-sdk/issues/1531) for more details and alternative approaches.
</Warning>
