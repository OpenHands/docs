...
Install the SDK packages in the Python environment that will run the server:
...
Clients must send the session key in the `X-Session-API-Key` header:
...
conversation.send_message("Create a TODO.md file with three setup tasks.")
conversation.run()
conversation.close()
```
