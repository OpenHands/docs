...
The Agent SDK serves as the **source of truth for agents** in OpenHands. The [OpenHands repository](https://github.com/All-Hands-AI/OpenHands) provides interfaces—web app, CLI, and cloud—that consume the SDK APIs. This architecture ensures consistency and enables flexible integration patterns.
- **Agent SDK = foundation.** The SDK defines all core components: agents, LLMs, conversations, tools, workspaces, events, and security policies.
...
graph TB
    ...
    SDK[Agent SDK<br><i>openhands.sdk + tools + workspace</i>]
...
- **[Tool System](/sdk/arch/tool-system):** Typed base class definitions for action, observation, tool, and executor
...
- **[MCP](/sdk/arch/mcp):** Model Context Protocol integration for external tools
...
**Key Components:**
- **[Execute Bash](/sdk/arch/execute-bash):** Shell command execution with persistent terminal sessions
- **[File Editor](/sdk/arch/file-editor):** String-replacement file editing with validation and undo
- **[Browser Use](/sdk/arch/browser-use):** Browser automation with Playwright integration
...
**Key Components:**
- **[Docker Workspace](/sdk/arch/docker-workspace):** Auto-spawns and manages agent-server in Docker containers with health checking
- **[Remote API Workspace](/sdk/arch/remote-api-workspace):** Connects to managed runtime services via Runtime API with resource scaling

**Design:** Both extend `RemoteWorkspace` from SDK, adding container lifecycle or API client functionality.
...
**Key Component:**
- **[Agent Server API](/sdk/arch/agent-server-api):** REST endpoints and WebSocket streaming for conversations, bash, files, events, desktop, and VSCode

**Features:**
- **REST API & WebSocket:** Create conversations, send messages, stream responses
- **Service Management:** ConversationService, BashService, EventService, DesktopService, VSCodeService
- **Session Management:** Isolated per-user sessions with API key authentication
- **Health Checking:** `/health` endpoint for container orchestration
...
**Tools Package:**
- [Execute Bash](/sdk/arch/execute-bash) – Terminal session management
- [File Editor](/sdk/arch/file-editor) – File manipulation tool
- [Browser Use](/sdk/arch/browser-use) – Browser automation

**Workspace Package:**
- [Docker Workspace](/sdk/arch/docker-workspace) – Containerized execution
- [Remote API Workspace](/sdk/arch/remote-api-workspace) – Managed runtime API

**Agent Server:**
- [Agent Server API](/sdk/arch/agent-server-api) – REST/WebSocket server
