...
- `Action` and `Observation` are Pydantic models; tool inputs are validated before execution, and outputs are coerced to the declared observation model (if present).
