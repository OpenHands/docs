...
description: Understand how the Agent SDK manages language models and how OpenHands interfaces build on top of it.
---
...
The OpenHands repository (UI/CLI) now consumes these SDK APIs. When you save LLM
settings in the UI they are stored using the same environment variable schema
and serialized into `LLM` objects before each run. That means any customization
documented here automatically applies to the OpenHands products.
