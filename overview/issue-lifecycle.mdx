...
<Tabs>
  <Tab title="OpenHands/OpenHands (monorepo)">
    ...
    <Note>
      An issue filed outside the web form (for example via the GitHub CLI or API) starts with no labels. The all-hands-bot app usually adds a type label within about an hour here too — but if it abstains, the issue waits for a human triager. Only once a type label is present does the readiness check run.
    </Note>
  </Tab>
