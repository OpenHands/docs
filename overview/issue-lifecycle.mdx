...
<Tabs>
  <Tab title="OpenHands/OpenHands (monorepo)">
    ...
    3. **Readiness check.** Once a type label is present, the issue readiness workflow evaluates the body against the type-specific criteria below and applies the `ready-for-dev` label within seconds if they are met.

    <Note>
      There is no auto-label workflow in the monorepo. An issue filed outside the web form (for example via the GitHub CLI or API) starts with no labels and waits for a human triager to add a type label. Only then does the readiness check run.
    </Note>
  </Tab>
  <Tab title="OpenHands/software-agent-sdk">
    1. **`needs-triage` on open.** A workflow adds the `needs-triage` label when the issue is opened, unless the issue already carries a type or priority label.
    2. **Type, topic, and priority labels.** The all-hands-bot app applies a type label (`bug` or `enhancement`) plus topic and priority labels, typically within about an hour.
    3. **Readiness check.** As soon as the type label lands, the issue readiness workflow evaluates the body and applies `ready-for-dev` within seconds if the criteria below are met.

    <Note>
      The bot can abstain from assigning a type label when it cannot classify the issue confidently. If your issue sits at `needs-triage` with no type label, the reliable remedy is to recreate it through the web issue form, which sets the type label at creation.
    </Note>
  </Tab>
</Tabs>
...
- **Monorepo issues filed via CLI or API** remain unlabeled until a human triages them.
...
<Warning>
  Only `###` headings are parsed. If you write the sections as `##` (h2) headings, every section parses as empty and the check fails silently — the issue simply never gets `ready-for-dev`. Keep the `###` headings exactly as the form renders them.
</Warning>

### Bug Reports

An issue labeled `bug` is ready for development when all three of the following hold:
...
- **Using `##` instead of `###` headings in an issue.** The readiness parser only reads `###` headings; `##` sections parse as empty and the check fails without explanation. See [Readiness Criteria](#readiness-criteria).
...
- **Linking a closed issue.** A closed issue does not satisfy the linked-issue requirement even if it carried `ready-for-dev`, and the validation error does not say that is the reason — it reports only that no linked issue carries the label.
